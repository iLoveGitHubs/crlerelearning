...
namespace Robotics
{
    public partial class Robotics : Form
    {
        private int M = 12, N = 18;
        private int deltaX, deltaY;
        private Point pri = new Point(5, 5);
        ...
        private void btnRuning_Click(object sender, EventArgs e)
        {
            ...
            //Dựa cơ sở tri thức để tìm đường đi
        }
        ...
        private void timerLoangDaTam_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                Before.X = Path[0].X;
                Before.Y = Path[0].Y;
                graphics = image.Draw_Bitmap_Graphics(5 + Path[0].Y * deltaY + 1, 5 + Path[0].X * deltaX + 1, "Image/Robot-48.png", graphics);
                timerPath.Start();
            }
        }
        ...
        private void timerPath_Tick(object sender, EventArgs e)
        {
            ...
            if (indexPath < Path.Count)
            {
                ...
                Before.Y = v;
                if(!dded[u,v])
                    graphics = image.Draw_Bitmap_Graphics(5 + v * deltaY + 1, 5 + u * deltaX + 1, "Images/1-cham.png", graphics);
                else
                    graphics = image.Draw_Bitmap_Graphics(5 + v * deltaY + 1, 5 + u * deltaX + 1, "Images/2-cham.png", graphics);
                dded[u, v] = true;
                indexPath++;
            }
            else
        ...
        private void DisplayArray(int m, int n, int[,] arr)
        {
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
